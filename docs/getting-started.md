...
`magento2-tools` is a **Claude Code plugin**: a set of 36 *skills* (structured,
...
- **Code-writing skills gate on your approval.** `feature` won't
  ...
  for you to literally type `release` before pushing anything. `remediate` takes **one**
  approval per batch of findings — never one per finding, and never any for an item it has
  flagged as needing a human (a leaked credential needs rotating, not deleting).
