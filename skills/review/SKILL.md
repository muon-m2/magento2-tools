...
5. Produce a report.
    ...
    - For JSON output (default when invoked from another skill or with `--format=json`):
      ...
      pipe it through `${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/emit-json.sh`. Writes to
      ...
      `DOCS_ROOT="${DOCS_ROOT_ARG:-.docs}" bash "${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/emit-json.sh"`
    ...
    - For SARIF output (CI / GitHub Code Scanning): run `${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/emit-sarif.sh` on the JSON
...
to `{ctx.docs_root}/reviews/`. Orchestrators such as `feature`
...
- `${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/emit-json.sh`: writes the findings JSON document per
  the shared schema (`context/references/findings-schema.md`). Owned by the `context` hub
  and reused by every findings-emitting skill.
- `${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/emit-sarif.sh`: converts a JSON document to SARIF 2.1.0 for GitHub Code Scanning.
