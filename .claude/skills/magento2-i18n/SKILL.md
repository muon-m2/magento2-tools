...
Run `scripts/merge-csv.sh <fresh.csv> <locale.csv>` for each target locale. The script
guarantees:
...
- `scripts/extract.sh` — Magento CLI wrapper or regex fallback.
- `scripts/merge-csv.sh` — deterministic merge: preserves translations, appends new
  phrases, separates obsolete phrases to `<locale>.obsolete.csv`.
- `scripts/validate-csv.sh` — CSV well-formedness + placeholder check.
