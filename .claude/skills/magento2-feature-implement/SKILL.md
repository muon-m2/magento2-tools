...
- `scripts/smoke-baseline.sh` — S1 baseline capture.
- `scripts/smoke-tail-since.sh` — S8 diff.
- `scripts/smoke-browser.mjs` — browser commands for S3–S7.
...
- `scripts/smoke-baseline.sh`: S1 — capture `var/log/exception.log` baseline.
- `scripts/smoke-tail-since.sh`: S8 — diff `var/log/exception.log` since baseline.
- `scripts/smoke-browser.mjs`: S3–S7 — headless browser driver (Playwright → Puppeteer → CDP).
