...
Run via `scripts/static-perf.sh <module-path>`.
...
Run via `scripts/runtime-checks.sh`.
...
1. **JSON + SARIF** (automated). `scripts/build-findings.sh` aggregates `static-perf.sh`
...
SCAN_ROOT=src/app/code INCLUDE_RUNTIME=0 \
bash .claude/skills/magento2-performance-audit/scripts/build-findings.sh
```
...
- `scripts/static-perf.sh` — static pattern scan over module sources.
- `scripts/runtime-checks.sh` — bin/magento + redis-cli + mysql probes.
- `scripts/build-findings.sh` — aggregate scanner output; emit JSON + SARIF via the
