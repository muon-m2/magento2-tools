...
# under .claude/skills/ MUST match the current version in
...
REGISTRY=".claude/skills/magento2-context/references/skill-versioning.md"
if [ ! -f "$REGISTRY" ]; then
...
# Walk every file under .claude/skills (except _tests) and find skill@version tokens.
...
# When a file lives under .claude/skills/<skill>/, the version it declares should match
