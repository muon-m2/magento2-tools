...
See `references/log-locations.md` and `scripts/log-triage.sh`.
...
See `references/event-catalog.md` and `scripts/plugin-trace.sh`.
...
See `references/di-graph-walk.md` and `scripts/di-walk.sh`.
...
See `references/slow-query-patterns.md` and `scripts/slow-query-parse.sh`.
...
- `scripts/log-triage.sh` — group log entries by signature.
- `scripts/di-walk.sh` — di.xml graph walker.
- `scripts/plugin-trace.sh` — plugin discovery for a target.
- `scripts/slow-query-parse.sh` — MySQL slow-log parser.
