...
3. Run optional static tool passes.
    ...
    - Use `scripts/static-tool-probe.sh` to list available tools and suggested commands without assuming they exist.
    - Use `scripts/discover-module.sh <module-path>` for a quick source inventory.
    - Use `scripts/collect-evidence.sh <module-path>` for grep-based risk and surface scans.
...
5. Produce a report.
    ...
    - For JSON output (default when invoked from another skill or with `--format=json`):
      ...
      pipe it through `scripts/emit-json.sh`. Writes to `.docs/reviews/{Vendor}_{Module}-review-{date}.json`.
    - For SARIF output (CI / GitHub Code Scanning): run `scripts/emit-sarif.sh` on the JSON
...
- Run `scripts/diff-scope.sh <module-path> <ref>` (default ref `origin/main`) to obtain
...
- `scripts/emit-json.sh`: writes the findings JSON document per the shared schema
  (`magento2-context/references/findings-schema.md`).
- `scripts/emit-sarif.sh`: converts a JSON document to SARIF 2.1.0 for GitHub Code Scanning.
- `scripts/diff-scope.sh`: lists files changed in a module since a git ref (powers diff mode).
