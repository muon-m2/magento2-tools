...
Run each command in order via `scripts/execute-plan.sh` or by direct shell:
...
"needs investigation" finding. Run via `scripts/smoke.sh`.
...
- `scripts/preflight.sh` — run pre-flight checks; emit JSON pass/fail summary.
- `scripts/execute-plan.sh` — execute a plan file step-by-step, log per-step results.
- `scripts/smoke.sh` — run smoke tests for given module list.
- `scripts/snapshot.sh` — create a pre-deploy snapshot of `generated/`, `var/`, optionally `vendor/`.
